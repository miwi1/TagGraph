...
FM::build(uint8_t* T,uint32_t n,uint32_t samplerate) {
    ...
    if( divsufsort(X,SA,n) != 0 ) {
      printf("FAILED DIVSUFSORT ERR CODE %i\n", divsufsort(X,SA,n));
...
FM::count(char* pattern) {
  ...
  while (sp<=ep && i>=1) { 
    ...
    uint32_t end_pos_rank = T_bwt->rank(c,ep);
    /* covers rare cases where c has rank 0 at the ep position */
    if (end_pos_rank == 0) {
      return 0;
    }

    sp = C[c] + T_bwt->rank(c, sp-1); /* LF Mapping */
    ep = C[c] + end_pos_rank-1; /* LF Mapping */
  }
...
uint32_t
FM::count_max_suffix(char* pattern, uint32_t* max_suffix_length) {
  uint32_t m = strlen(pattern);
  uint8_t c = remap[pattern[m-1]]; /* map pattern to our alphabet */
  *max_suffix_length = 0;
  if (c==0) { return 0; } /* return 0 if last amino acid of pattern is not in index */
...
std::vector<uint32_t>
FM::locate(char* pattern, uint32_t* matches) {
  ...
  if (sp<=ep) {
    ...
    *matches = ep-sp+1;
    ...
    std::vector<uint32_t> v(locations, locations + *matches);
    return v;
  } else {
    /* no matches */
    *matches = 0;
...
/* Text starts at index position 0, stop is index position after string end */
char*
FM::extract(uint32_t start,uint32_t stop)
{
    ...
    if(stop > (this->n-1) ) stop = n-1; 
    if(start >= stop) {
      return (char*)NULL;
    }
	
    m = stop-start; /* snippet len */
    T = (uint8_t*) safe_malloc( (m+1) * sizeof(uint8_t)  );
    
    /* determine start pos of backwards search */
    skip = (stop - 1)%samplerate;

    /* determine distance from start pos to the text snippet we want */
    if (stop/samplerate == (n-1)/samplerate) 
      skip = n-1 - stop;
    else {
      /* Find distance from next samplerate interval */
      skip = samplerate - skip - 1;
    }

    /* note that if (stop-1)%samplerate == 0, we have to go to the suffix index one to the right in order to get the identity of the character at position stop-1 using the burrows-wheeler transform, can also get this character from the run-length encoded C array but the code is more elegant this way */
    j = positions[(stop-1)/samplerate+1];
